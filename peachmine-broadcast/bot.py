...
ADMIN_IDS = [
    ...
    985239713751384094  # Второй админ
]
...
async def restart_cmd(ctx: discord.ApplicationContext):
    ...
    try:
        for admin_id in ADMIN_IDS:
            ...
            await admin.send(embed=create_embed(
                "Перезагрузка",
                f"🔄 Бот **перезагружается**...\n\n"
                f"⏱️ Аптайм до перезагрузки: **{get_uptime()}**\n"
    ...
    print(f"\n[RESTART] Перезагрузка по команде от {ctx.author}...")
    RESTART_FLAG = True
    await bot.close()
...
async def console_listener():
    ...
    while True:
        try:
            ...
            if cmd == "restart":
                print("[CONSOLE] Команда restart получена")
                ...
                try:
                    for admin_id in ADMIN_IDS:
                        ...
                        await admin.send(embed=create_embed(
                            "Перезагрузка",
                            f"🔄 Бот **перезагружается** (консольная команда)...\n\n"
                            f"⏱️ Аптайм: **{get_uptime()}**\n"
                            f"🕐 Время: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}",
                            WARNING_COLOR
                        ))
                except:
                ...
                print("  restart - перезагрузить бота")
...
async def on_ready():
    ...
    print(f"{'═'*50}")
    print(f"  Консольные команды: restart, stop, status, help")
    print(f"{'═'*50}\n")
...
if __name__ == "__main__":
    ...
    # После закрытия бота - выходим с кодом для перезапуска
    if RESTART_FLAG:
        print("\n[RESTART] Бот будет перезапущен...\n")
        sys.exit(0)  # start.bat перезапустит
    else:
        print("\n[STOP] Бот остановлен.\n")
        sys.exit(1)  # Выход без перезапуска
